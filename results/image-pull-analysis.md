...
> It explains the similar `Reported sizes` by the AKS image pull for the 2 `run48-*` images, while their uncompressed sizes are significantly larger: The added data difference of `2.50 GB` was generated by creating an empty file, that compresses extremely well (see Docker file `.\src\acr-runs-tests\dockerfile-10G-win2019`).
